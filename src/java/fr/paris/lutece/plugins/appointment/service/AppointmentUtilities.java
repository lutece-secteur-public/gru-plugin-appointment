...
public final class AppointmentUtilities {
						...
						int nbSlots = toIntExact(listSlots.stream()
								.filter(s -> (s.getStartingDateTime().toLocalDate().equals(startingDateOfPeriodToSearch)
										...
										&& (s.getStartingDateTime().toLocalDate().equals(endingDateOfPeriodToSearch)
												|| s.getStartingDateTime().toLocalDate()
														.isBefore(endingDateOfPeriodToSearch)))
								.count());
	...
	public static int checkAndReturnNbBookedSeats(String strNbBookedSeats, AppointmentForm form,
		...
		if (StringUtils.isNotEmpty(strNbBookedSeats)) {
			try {
				...
				GenericAttributeError genAttError = new GenericAttributeError();
				genAttError
						.setErrorMessage(I18nService.getLocalizedString(ERROR_MESSAGE_FORMAT_NB_BOOKED_SEAT, locale));
				listFormErrors.add(genAttError);
			}
		}
	...
	public static String[][] getPermissions(List<AppointmentForm> listForms, AdminUser user) {
		...
		for (AppointmentForm tmpForm : listForms) {
			...
			strRetour[0] = String.valueOf(
					RBACService.isAuthorized(AppointmentForm.RESOURCE_TYPE, String.valueOf(tmpForm.getIdForm()),
			...
			strRetour[3] = String.valueOf(RBACService.isAuthorized(AppointmentForm.RESOURCE_TYPE,
					String.valueOf(tmpForm.getIdForm()), AppointmentResourceIdService.PERMISSION_MODIFY_FORM, user));
			strRetour[4] = String.valueOf(RBACService.isAuthorized(AppointmentForm.RESOURCE_TYPE,
					String.valueOf(tmpForm.getIdForm()), AppointmentResourceIdService.PERMISSION_COPY_FORM, user));
			strRetour[5] = String.valueOf(RBACService.isAuthorized(AppointmentForm.RESOURCE_TYPE,
					String.valueOf(tmpForm.getIdForm()), AppointmentResourceIdService.PERMISSION_CHANGE_STATE, user));
			strRetour[6] = String.valueOf(RBACService.isAuthorized(AppointmentForm.RESOURCE_TYPE,
	 ...
	 *            the date of modification (date of apply of the new week
	 *            definition)
	...
	public static boolean checkNoAppointmentsImpacted(List<Appointment> listAppointment, int nIdForm,
			LocalDate dateOfModification, AppointmentForm appointmentForm) {
		...
		AppointmentForm previousAppointmentForm = FormService.buildAppointmentForm(nIdForm,
	 ...
	 * @param slot
	 *            the slot
	 * @return true if there are no validated appointments on this slot, false
	 *         otherwise
	 */
	public static boolean checkNoValidatedAppointmentsOnThisSlot(Slot slot) {
		...
		if (CollectionUtils.isNotEmpty(listAppointmentsOnThisSlot)) {
			listAppointmentsOnThisSlot = listAppointmentsOnThisSlot.stream().filter(a -> a.getIsCancelled() == false)
					.collect(Collectors.toList());
		}
		if (CollectionUtils.isNotEmpty(listAppointmentsOnThisSlot)) {
	 ...
	 * @param nIdWeekDefinition the week definition id
	 * @param bShiftSlot the boolean value for the shift
	...
	public static List<Slot> findSlotsImpactedByThisTimeSlot(TimeSlot timeSlot, int nIdForm, int nIdWeekDefinition,
		...
		if (maxDate != null && !maxDate.isBefore(beginDate)) {
			...
			List<Slot> listSlots = SlotService.findSlotsByIdFormAndDateRange(nIdForm,
					beginDate.atStartOfDay(), maxDate.atTime(LocalTime.MAX));
			...
			if (bShiftSlot) {
				listSlotsImpacted = listSlots.stream()
						.filter(slot -> ((slot.getStartingDateTime().getDayOfWeek() == DayOfWeek
								.of(workingDay.getDayOfWeek())) && (!slot.getStartingTime()
										.isBefore(timeSlot.getStartingTime()) || (slot.getStartingTime()
												.isBefore(timeSlot.getStartingTime())
				...
				listSlotsImpacted = listSlots.stream()
						.filter(slot -> ((slot.getStartingDateTime().getDayOfWeek() == DayOfWeek
								...
								&& (slot.getStartingTime().equals(timeSlot.getStartingTime())
										|| (slot.getStartingTime()
												.isAfter(timeSlot.getStartingTime())
