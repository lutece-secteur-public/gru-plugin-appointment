...
public final class WeekDefinitionService {
	...
	/**
	 * Remove the weekdefinition (and by cascade the working days and the time
	 * slots)
	 * 
	...
	public static WeekDefinition findWeekDefinitionByIdFormAndClosestToDateOfApply(int nIdForm, LocalDate dateOfApply) {
		...
		if (closestDate == null) {
			...
			weekDefinition = listWeekDefinition.stream()
					.min((w1, w2) -> w1.getDateOfApply().compareTo(w2.getDateOfApply())).get();
		} else {
			weekDefinition = listWeekDefinition.stream().filter(x -> closestDate.isEqual(x.getDateOfApply())).findAny()
					.orElse(null);
		}
		if (weekDefinition != null) {
			weekDefinition.setListWorkingDay(
	...
	public static WeekDefinition findNextWeekDefinition(int nIdForm, LocalDate previousDateOfApply) {
		...
		if (CollectionUtils.isNotEmpty(listWeekDefinition)) {
			listWeekDefinition = listWeekDefinition.stream()
					.filter(x -> x.getDateOfApply().isAfter(previousDateOfApply)).collect(Collectors.toList());
			if (CollectionUtils.isNotEmpty(listWeekDefinition)) {
				nextWeekDefinition = listWeekDefinition.stream()
						.min((w1, w2) -> w1.getDateOfApply().compareTo(w2.getDateOfApply())).get();
			}
		}
	 ...
	 * @param nIdForm
	 *            the form Id
	 * @return a reference list of all the week definitions of a form (Id of the
	 *         week definition / date of apply of the week definition
	 */
	public static ReferenceList findAllDateOfWeekDefinition(int nIdForm) {
		...
		for (WeekDefinition weekDefinition : listWeekDefinition) {
			listDate.addItem(weekDefinition.getIdWeekDefinition(),
					weekDefinition.getDateOfApply().format(Utilities.getFormatter()));
		}
		return listDate;
	}
	 ...
	 * @param nIdForm
	 *            the form id
	 * @return a HashMap with the date of apply in key and the week definition
	 *         in value
	 */
