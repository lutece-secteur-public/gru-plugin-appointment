					...
					while (timeTemp.isBefore(maxTimeForThisDay) || !timeTemp.equals(maxTimeForThisDay)) {
						...
						if (mapSlot.containsKey(dateTimeTemp)) {
							...
							if (timeSlot != null) {
								...
								slotToAdd = buildSlot(nIdForm, new Period(dateTimeTemp, dateTemp.atTime(timeTemp)), nMaxCapacity,
										nMaxCapacity, nMaxCapacity, timeSlot.getIsOpen(), Boolean.FALSE);
								listSlot.add(slotToAdd);
							} else {
								break;
							}
	...
	private static List<Slot> generateListSlotToCreateAfterASlot(Slot slot) {
		...
		while (!endingDateTime.isAfter(endingDateTimeOfTheDay)) {
			Slot slotToCreate = buildSlot(nIdForm, new Period(startingDateTime, endingDateTime), nMaxCapacity,
					nMaxCapacity, nMaxCapacity, Boolean.TRUE, Boolean.TRUE);
