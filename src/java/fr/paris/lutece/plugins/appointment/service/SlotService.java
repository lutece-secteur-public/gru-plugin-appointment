...
public final class SlotService {
			...
			if (!bShifSlot) {
				...
				if (CollectionUtils.isNotEmpty(listStartingDateTimeNextSlot)) {
					...
					if (workingDay != null) {
						...
						// slot
						listSlotToCreate
								.addAll(generateListSlotToCreateAfterATime(slot.getEndingDateTime(), slot.getIdForm()));
					}
				}
				...
				if (nextStartingDateTime != null && !slot.getEndingDateTime().isEqual(nextStartingDateTime)) {
					Slot slotToCreate = buildSlot(slot.getIdForm(),
							new Period(slot.getEndingDateTime(), nextStartingDateTime), slot.getMaxCapacity(),
				...
				List<Slot> listSlotToDelete = SlotService.findSlotsByIdFormAndDateRange(
	...
	public static LocalDate findFirstDateOfFreeOpenSlot(int nIdForm, LocalDate startingDate, LocalDate endingDate) {
		...
		while (!bFreeSlotFound
				&& (currentDateOfSearch.isBefore(endingDate) || !currentDateOfSearch.isAfter(endingDate))) {
