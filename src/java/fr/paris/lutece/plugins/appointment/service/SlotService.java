...
public final class SlotService {
			...
			if (workingDay != null) {
				...
				if (listDateOfClosingDay.contains(dateTemp)) {
					listSlot.add(buildSlot(nIdForm,
							...
							nMaxCapacity, nMaxCapacity, nMaxCapacity, Boolean.FALSE, Boolean.FALSE));
				} else {
						...
						if (mapSlot.containsKey(dateTimeTemp)) {
							...
							if (timeSlot != null) {
								...
								slotToAdd = buildSlot(nIdForm, new Period(dateTimeTemp, dateTemp.atTime(timeTemp)),
										nMaxCapacityToPut, nMaxCapacityToPut, nMaxCapacityToPut, timeSlot.getIsOpen(),
										Boolean.FALSE);
								listSlot.add(slotToAdd);
							} else {
								break;
	 ...
	 *            the previous ending time (null if the ending time is
	 *            unchanged)
	 * @param bShifSlot
	...
	private static void updateSlotWithoutShift(Slot slot) {
		...
		if (nextStartingDateTime != null && !slot.getEndingDateTime().isEqual(nextStartingDateTime)) {
			Slot slotToCreate = buildSlot(slot.getIdForm(), new Period(slot.getEndingDateTime(), nextStartingDateTime),
					slot.getMaxCapacity(), slot.getMaxCapacity(), slot.getMaxCapacity(), Boolean.FALSE, Boolean.TRUE);
	...
	private static void updateSlotWithShift(Slot slot, LocalTime previousEndingTime) {
		...
		// Need to order the list of slot to shit according to the shift
	...
	private static List<Slot> generateListSlotToCreateAfterATime(LocalDateTime dateTimeToStartCreation, int nIdForm) {
		...
		while (!endingDateTime.isAfter(endingDateTimeOfTheDay)) {
			Slot slotToCreate = buildSlot(nIdForm, new Period(startingDateTime, endingDateTime), nMaxCapacity,
					nMaxCapacity, nMaxCapacity, Boolean.FALSE, Boolean.TRUE);
